...
def svm_theta(
        X, Y, n_samples=50, subsets_size_range=(2, 8),
    ...
    Gx = graph(X)
    Gy = graph(Y)
    return svm_theta_pair(Gx, Gy, n_samples, subsets_size_range, metric)


def svm_theta_pair(
        Gx, Gy, n_samples=50, subsets_size_range=(2, 8),
        metric=(lambda x, y: x*y)):
    """SVM theta kernel as proposed in :cite:`Johansson2015LearningWS`.
    ...
    Ldx = Gx.calculate_subgraph_samples_metric_dictionary(
        "svm", n_samples=n_samples, subsets_size_range=subsets_size_range)
