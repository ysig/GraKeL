...
def neighborhood_subgraph_pairwise_distance(X, Y, Lx, Ly, LEx, LEy, r=3, d=4):
    """Neighborhood subgraph pairwise distance kernel :cite:`Costa2010FastNS`.
    ...
    Gx = graph(X, Lx, LEx)
    Gy = graph(Y, Ly, LEy)
    return neighborhood_subgraph_pairwise_distance_pair(Gx, Gy, r=r, d=d)


def neighborhood_subgraph_pairwise_distance_pair(Gx, Gy, r=3, d=4):
    """Neighborhood subgraph pairwise distance kernel :cite:`Costa2010FastNS`.
...
def hash_neighborhoods(G, N, D_pair, r, purpose="adjacency"):
    ...
    D_pairs : dict
        A dictionary that maps edges (tuple pairs of vertex symbols) to element
        distances (int - as produced from a BFS traversal).
...
def as_n_bit(num, n):
    """Keep the n first bits of an integer.
...
def APHash(string):
    """Arash Partov hashing as in the original implementation of NPSDK.
    ...
    for s in list(string):
        if hash_num & 1 == 0:
            hash_num ^= \
                as_n_bit((hash_num << 7), n) ^\
                as_n_bit(as_n_bit(ord(s), n) * as_n_bit((hash_num >> 3), n), n)
        else:
            hash_num ^= ~as_n_bit(
                (as_n_bit((hash_num << 11), n) +
                    (as_n_bit(ord(s), n) ^ as_n_bit((hash_num >> 5), 2))), n)
