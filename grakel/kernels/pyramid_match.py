...
def pyramid_match(X, Y, Lx, Ly, L=4, d=6):
    """Pyramid match kernel as defined in :cite:`Nikolentzos2017MatchingNE`.
    ...
    return float(pyramid_match_matrix(
        {0: Gx}, {0: Gy}, with_labels=with_labels, L=L, d=d)[0, 0])


def pyramid_match_matrix(Graphs_x, Graphs_y=None, with_labels=True, L=4, d=6):
    """Pyramid match kernel function as in :cite:`Nikolentzos2017MatchingNE`.
            ...
            for j in range(L):
                ...
                for p in range(Us[i].shape[0]):
                    ...
                    for q in range(Us[i].shape[1]):
                        ...
                        if with_labels:
                            D[labels[g.label(p, purpose='adjacency')]*d +
                                q, int(T[p, q])] = \
                                D[labels[g.label(p, purpose='adjacency')]*d +
                                    q, int(T[p, q])] + 1
                        else:
