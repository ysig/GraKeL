...
def propagation(X, Y, Lx, Ly, Tx=None, Ty=None, t_max=5, w=10, M="TV",
                base_kernel=(lambda x, y: sum(a*b for (a, b) in zip(x, y)))):
    r"""Propagation kernel for fully labeled graphs.
    ...
    T{x,y} : np-array, default=None
        Transition matrices corresponding to Graphs X, Y
    ...
    base_kernel : function (list, list -> number),
    ...
    Gx = graph(X, Lx)
    Gy = graph(Y, Ly)
    return float(propagation_matrix({0: Gx}, {0: Gy}, t_max=t_max,
                 T={0: Tx, 1: Ty}, w=w, M=M, base_kernel=base_kernel)[0, 0])


def propagation_matrix(
    Graphs_x, Graphs_y=None, t_max=5, T=None, w=10, M="TV",
        base_kernel=(lambda x, y: sum(a*b for (a, b) in zip(x, y)))):
    r"""Propagation kernel for fully labeled graphs.

    See :cite:`Neumann2015PropagationKE`: p. 216, 221.
            ...
            for i in range(0, ng):
                if T[i] is not None:
                    ...
                    if T[i].shape[0] != T.shape[1]:
                        raise ValueError('transition matrix \
                            must be a square matrix')
                    if T[i].shape[0] != g_iter[i].nv():
                        raise ValueError('propagation matrix \
