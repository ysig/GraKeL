...
def multiscale_laplacian(X, Y, Phi_x, Phi_y, L=3, gamma=0.01):
    ...
    Gx = graph(X, Phi_x)
    Gy = graph(Y, Phi_y)
    return multiscale_laplacian_pair(Gx, Gy, gamma=gamma)


def multiscale_laplacian_pair(Gx, Gy, L=3, gamma=0.01):
    """Laplacian Graph Kernel as proposed in :cite:`Kondor2016TheML`.

    Parameters
    ----------
    G{x,y} : graph
        The pair of graphs on which the kernel is applied with feature vectores
        as node labels.
    ...
    pick = lambda node, level:\
        ...
        if node < Gx.n else\
        ([idx+Gx.n for idx in Ny[level][node-Gx.n]],
            laplacian(Ay[Ny[level][node-Gx.n], :][:, Ny[level][node-Gx.n]]))
    ...
    for l in range(1, L+1):
        ...
        for i in range(0, gram_matrix_size):
            ...
            for j in range(0, gram_matrix_size):
                ...
                gram_matrix[i, j] =\
                    generalized_FLG_core(La, Lb, extracted_gm, gamma=gamma)
