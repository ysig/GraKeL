...
def hadamard_code(X, Y, Lx, Ly, base_kernel, niter=5, hc_type='simple',
                  **kwargs):
    ...
    return float(hadamard_code_matrix({0: Gx}, bkm, {0: Gy}, niter=5,
                                      hc_type=hc_type, **kwargs)[0, 0])


def hadamard_code_matrix(Graphs_x, base_kernel_matrix, Graphs_y=None, niter=5,
                         hc_type='simple', **kwargs):
    """Hadamard code kernel, for matrices, as derived from :cite:`icpram16`.
    ...
    Graphs_{x,y} : dict, default_y=None
        ...
        number of values.
        If value of Graphs_y is None the kernel matrix is computed between
        all pairs of Graphs_x, where in another case the kernel_matrix rows
        correspond to elements of Graphs_y, and columns to the elements
        of Graphs_x.
    ...
    if hc_type == 'simple':
        ...
        L = kwargs.get('L', 4)
        if L <= 0:
            raise ValueError('L must be a positive integer as it corresponds\
    ...
    if Graphs_x is None:
        ...
        g_iter = itertools.chain([Graphs_x[i] for i in range(0, ng_a)],
                                 [Graphs_y[i] for i in range(0, ng_b)])
    ...
    if shortened:
        if rho == -1:
            rho = int(L/(ord_H-1))
            if rho < 0:
                raise ValueError('The default calculated rho is too small,\
                                  raise more L ~ L must approach the number\
                                  of labels ('+str(ord_H))

        elif rho*(ord_H-1) > L:
            raise ValueError('rho*(ord_H-1)='+str(rho)+'*('+str(ord_H)+'-1)\
                              > L='+str(L))
        bit_array = dict()
    ...
    for (i, g) in enumerate(g_iter):
        ...
        for v in orig_labels[i].keys():
            new_labels[v] = get(Labeling, labels_enum[orig_labels[i][v]])
